...
func TestConfigureTextToSpeech_PaceAbsentByDefault(t *testing.T) {
	cred := newVaultCredential(map[string]interface{}{"key": "k"})
	opt, _ := NewSarvamOption(newTestLogger(), cred, utils.Option{})
	data := opt.configureTextToSpeech()["data"].(map[string]interface{})

	_, present := data["pace"]
	assert.False(t, present, "pace must not be sent unless configured")
}

func TestConfigureTextToSpeech_PaceOverride(t *testing.T) {
	cred := newVaultCredential(map[string]interface{}{"key": "k"})
	opts := utils.Option{"speak.model": "bulbul:v3", "speak.pace": 0.85}
	opt, _ := NewSarvamOption(newTestLogger(), cred, opts)
	data := opt.configureTextToSpeech()["data"].(map[string]interface{})

	assert.Equal(t, 0.85, data["pace"])
}

func TestConfigureTextToSpeech_PaceFromString(t *testing.T) {
	// the UI persists options as text, so pace arrives as a string
	cred := newVaultCredential(map[string]interface{}{"key": "k"})
	opts := utils.Option{"speak.model": "bulbul:v3", "speak.pace": "0.9"}
	opt, _ := NewSarvamOption(newTestLogger(), cred, opts)
	data := opt.configureTextToSpeech()["data"].(map[string]interface{})

	assert.Equal(t, 0.9, data["pace"])
}

func TestConfigureTextToSpeech_PaceClampedPerModel(t *testing.T) {
	cred := newVaultCredential(map[string]interface{}{"key": "k"})

	// v3 floor is 0.5; an out-of-range value would make Sarvam reject the
	// config message and the call would go silent.
	opts := utils.Option{"speak.model": "bulbul:v3", "speak.pace": 0.1}
	opt, _ := NewSarvamOption(newTestLogger(), cred, opts)
	data := opt.configureTextToSpeech()["data"].(map[string]interface{})
	assert.Equal(t, 0.5, data["pace"])

	// v2 allows slower speech than v3
	opts = utils.Option{"speak.model": "bulbul:v2", "speak.pace": 0.1}
	opt, _ = NewSarvamOption(newTestLogger(), cred, opts)
	data = opt.configureTextToSpeech()["data"].(map[string]interface{})
	assert.Equal(t, 0.3, data["pace"])

	// ceiling
	opts = utils.Option{"speak.model": "bulbul:v3", "speak.pace": 5}
	opt, _ = NewSarvamOption(newTestLogger(), cred, opts)
	data = opt.configureTextToSpeech()["data"].(map[string]interface{})
	assert.Equal(t, 2.0, data["pace"])
}
